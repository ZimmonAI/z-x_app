import { z } from 'zod';
export const SAFE_ERROR_FAMILIES = ['invalid-owner-request','route-not-found','route-deactivated','route-parameter-validation','no-eligible-capacity','login-required','account-attention','adapter-unavailable','provider-rejected','timeout','cancelled','malformed-output','storage-input-failure','storage-output-failure','callback-failure','reconciliation-required','internal-safe-failure'] as const;
