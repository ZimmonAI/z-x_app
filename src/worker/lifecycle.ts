...
export async function executeFixturePath(
  ...
  let outcome = 'failed';
  try {
    const result = await executePreparedFixturePath(
      request,
      executionId,
      route,
      capacity,
    ...
    await dependencies.capacity.reportOutcome(
      { leaseRef: capacity.leaseRef, outcome },
      signal,
    );
...
async function acquirePreparationClaim(
  ...
  workerId: string,
  leaseSeconds: number,
): Promise<PreparationClaim | null> {
  return withTransaction(pool, async (client) => {
    const selected = await client.query<{
      execution_id: string;
      status: 'accepted' | 'waiting-capacity';
      current_attempt_number: number;
      request_envelope: unknown;
      trace_id: string;
      attempt_id: string | null;
      route_snapshot: RouteSnapshotV1 | null;
    }>(
      `select e.id as execution_id, e.status, e.current_attempt_number,
    ...
    if (row.status === 'accepted') {
      ...
      await client.query(
        ...
        [attemptId, row.execution_id, attemptNumber, workerId, leaseToken, leaseSeconds],
      );
      await client.query(
        `update execution.executions
          ...
          where id=$1`,
        [row.execution_id, attemptNumber],
      );
      ...
      const claimed = await client.query(
        `update execution.execution_attempts
          ...
          returning id`,
        [attemptId, workerId, leaseToken, leaseSeconds],
      );
...
async function persistPreparationFailure(
  ...
  await withTransaction(pool, async (client) => {
    const stateResult = await client.query<{ status: string }>(
      'select status from execution.executions where id=$1 for update',
      [claim.executionId],
    );
    const currentState = stateResult.rows[0]?.status;
    if (!currentState) throw new Error('execution disappeared during preparation');
    ...
    if (!attempt.rowCount) throw new Error('preparation lease lost');
...
export async function prepareNextExecution(
  pool: pg.Pool,
  workerId: string,
  leaseSeconds: number,
  dependencies: FixtureDependencies,
  ...
  try {
      ...
      await withTransaction(pool, async (client) => {
        const attempt = await client.query(
          `update execution.execution_attempts
            ...
            where id=$1 and execution_id=$2 and lease_token=$3 and lease_expires_at>now()
            returning id`,
          [claim.attemptId, claim.executionId, claim.leaseToken, route],
        );
        if (!attempt.rowCount) throw new Error('preparation lease lost');
        await client.query(
          `update execution.executions
              set status='waiting-capacity', lock_version=lock_version+1, updated_at=now()
            where id=$1 and status='resolving-route'`,
          [claim.executionId],
        );
        await appendTransition(client, {
          executionId: claim.executionId,
    ...
    await withTransaction(pool, async (client) => {
      const attempt = await client.query(
        `update execution.execution_attempts
          ...
          where id=$1 and execution_id=$2 and lease_token=$3 and lease_expires_at>now()
          returning id`,
        [claim.attemptId, claim.executionId, claim.leaseToken, capacity],
      );
      if (!attempt.rowCount) throw new Error('preparation lease lost');
      await client.query(
        `update execution.executions
            set status='queued', lock_version=lock_version+1, updated_at=now()
          where id=$1 and status='waiting-capacity'`,
        [claim.executionId],
      );
      await appendTransition(client, {
        executionId: claim.executionId,
    ...
    await persistPreparationFailure(
      pool,
      claim,
      toSafeError(error, claim.request.traceId),
      workerId,
    );
    return true;
  }
...
export async function completeClaimedExecution(
  pool: pg.Pool,
  claim: ClaimedExecution,
  workerId: string,
  dependencies: FixtureDependencies,
  ...
  const loaded = await pool.query<{
    request_envelope: unknown;
    route_snapshot: RouteSnapshotV1;
    capacity_snapshot: CapacitySnapshotV1;
    started_at: Date;
  }>(
    `select r.request_envelope, a.route_snapshot, a.capacity_snapshot, a.started_at
       from execution.executions e
       join execution.execution_requests r on r.id=e.request_id
       join execution.execution_attempts a on a.execution_id=e.id
      where e.id=$1 and a.id=$2 and a.lease_token=$3
        and a.lease_expires_at>now() and e.status='running' and a.status='running'`,
    [claim.executionId, claim.attemptId, claim.leaseToken],
  );
  ...
  try {
    const output = await executePreparedFixturePath(
      request,
      claim.executionId,
      row.route_snapshot,
    ...
    await withTransaction(pool, async (client) => {
      ...
      if (!attempt.rowCount) throw new Error('lease lost before success persistence');
      const execution = await client.query(
        `update execution.executions
          ...
          returning id`,
        [claim.executionId, result],
      );
      if (!execution.rowCount) throw new Error('execution state changed before success persistence');
      await appendTransition(client, {
        executionId: claim.executionId,
        attemptId: claim.attemptId,
        fromStatus: 'running',
    ...
    await withTransaction(pool, async (client) => {
      ...
      if (!attempt.rowCount) throw new Error('lease lost before failure persistence');
      await client.query(
        `update execution.executions
            set status=$2, error_envelope=$3, terminal_at=now(),
                lock_version=lock_version+1, updated_at=now()
          where id=$1 and status='running'`,
        [claim.executionId, terminalState, failure],
      );
      await appendTransition(client, {
        executionId: claim.executionId,
        attemptId: claim.attemptId,
      ...
      if (terminalState === 'reconciliation-required') {
        await client.query(
          ...
          [randomUUID(), claim.executionId, claim.attemptId, failure.family, { code: failure.code }],
        );
      }
    });
  } finally {
    await dependencies.capacity.reportOutcome(
      { leaseRef: row.capacity_snapshot.leaseRef, outcome: capacityOutcome },
      signal,
    );
    await dependencies.capacity.release(
      { leaseRef: row.capacity_snapshot.leaseRef },
      signal,
    );
  }
}
