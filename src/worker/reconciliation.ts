...
interface StorageCompletionClaim {
  executionId: string;
  attemptId: string;
  caseId: string;
  claimedUntil: Date;
...
function boundedSeconds(value: number, fallback: number): number {
...
async function claimStorageCompletion(
  pool: pg.Pool,
  leaseSeconds: number,
): Promise<StorageCompletionClaim | null> {
...
function validateReconciledStorageResult(
  ...
  const kind = result.mimeType.startsWith('image/')
    ...
    : result.mimeType.startsWith('video/')
      ? 'video'
      : undefined;
  if (!kind) throw new Error('reconciled storage result is not image or video media');
...
async function reopenStorageCase(
  pool: pg.Pool,
  claim: StorageCompletionClaim,
...
async function finalizeCompletedStorageResult(
  pool: pg.Pool,
  claim: StorageCompletionClaim,
  ...
  await withTransaction(pool, async (client) => {
    const selected = await client.query<{
      ...
      `select e.status as execution_status, a.status as attempt_status,
        ...
        where e.id=$1
        for update of e, a, c`,
      [claim.executionId, claim.attemptId, claim.caseId],
    );
    const row = selected.rows[0];
    if (!row) throw new Error('storage reconciliation state disappeared');
      ...
      if (!existing.success || JSON.stringify(existing.data) !== JSON.stringify(result)) {
        await client.query(
          `update execution.execution_reconciliation_cases
            ...
            where id=$1`,
          [claim.caseId, { conflict: true, code: 'ZX_STORAGE_RESULT_CONFLICT' }],
        );
      }
      return;
    }
    ...
    if (
      row.execution_status !== 'reconciliation-required' ||
    ...
    if (!attempt.rowCount) throw new Error('storage reconciliation attempt state changed');
    ...
    if (!execution.rowCount) throw new Error('storage reconciliation execution state changed');
    ...
    await client.query(
      `insert into execution.execution_status_transitions
       ...
       values ($1,$2,$3,'reconciliation-required','succeeded',
               'storage-completion-reconciled','worker',$4,$5,'{}'::jsonb)`,
      [randomUUID(), claim.executionId, claim.attemptId, workerId, claim.request.traceId],
    );

    const reconciliationCase = await client.query(
      `update execution.execution_reconciliation_cases
...
async function finalizeTerminalStorageFailure(
  pool: pg.Pool,
  claim: StorageCompletionClaim,
  errorCode: string,
  workerId: string,
): Promise<void> {
  const error: SafeErrorV1 = {
