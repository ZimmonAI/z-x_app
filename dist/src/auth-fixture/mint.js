...
import { FIXTURE_AUTH_ALLOWED_OWNER_APP, FIXTURE_AUTH_ALLOWED_SCOPES, FIXTURE_AUTH_MAX_TOKEN_TTL_SECONDS, } from './constants.js';
import { readEnvironmentFile } from './env-file.js';
function validateScopes(scopes) {
    if (scopes.length === 0)
        throw new Error('at least one --scope required');
    const allowed = new Set(FIXTURE_AUTH_ALLOWED_SCOPES);
...
export async function mintFixtureToken(options) {
...
export function parseMintArguments(arguments_) {
    ...
    if (!options.ownerApp)
        throw new Error('--owner-app required');
...
export async function runMintCommand(arguments_) {
