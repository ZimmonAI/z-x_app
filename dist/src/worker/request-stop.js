...
import { readWorkerStopAcknowledgement, removeWorkerStopAcknowledgement, removeWorkerStopRequestIfMatching, writeWorkerStopRequest, } from './control.js';
...
export async function requestWorkerStop(controlDir) {
    const request = {
...
export async function runWorkerStopCommand() {
    const controlDir = loadConfig().ZX_WORKER_CONTROL_DIR;
    if (!controlDir)
        throw new Error('ZX_WORKER_CONTROL_DIR required');
...
//# sourceMappingURL=request-stop.js.map