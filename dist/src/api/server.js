...
export async function buildServer(options) {
    ...
    app.addHook('onSend', async (_request, _reply, payload) => {
        const responseBytes = typeof payload === 'string' || Buffer.isBuffer(payload)
    ...
    app.setErrorHandler((error, _request, reply) => {
        ...
        const statusCode = explicitStatus ?? (error instanceof ZodError ? 400 : isDatabaseError(error) ? 503 : 500);
