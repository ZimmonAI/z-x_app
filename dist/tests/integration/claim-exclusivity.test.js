...
test('two workers cannot claim one prepared execution', async () => {
    const pool = testPool();
    await reset(pool);
    const submitted = await new ExecutionsRepository(pool).submit(validRequest());
    const executionId = submitted.id;
    const attemptId = randomUUID();
    await pool.query(`update execution.executions
        set status='queued', current_attempt_number=1
      where id=$1`, [executionId]);
    await pool.query(`insert into execution.execution_attempts
      (id, execution_id, attempt_number, status, route_snapshot,
       capacity_snapshot, started_at)
     values ($1,$2,1,'queued','{}'::jsonb,'{}'::jsonb,now())`, [attemptId, executionId]);
...
test('manual retry is prepared before it becomes claimable', async () => {
    ...
    const submitted = await repository.submit(validRequest());
    await pool.query(`update execution.executions
        set status='failed', terminal_at=now()
      where id=$1`, [submitted.id]);
    ...
    expect(await prepareManualRetry(pool, 'retry-preparer', 60, {
...
//# sourceMappingURL=claim-exclusivity.test.js.map