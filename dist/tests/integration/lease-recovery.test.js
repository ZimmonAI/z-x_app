...
test('expired pre-dispatch lease returns work to the queue', async () => {
    ...
    await pool.query(`insert into execution.execution_attempts
     ...
     values ($1,$2,1,'queued','{}'::jsonb,'{}'::jsonb,now())`, [randomUUID(), executionId]);
    const claim = await claimNext(pool, 'worker', 60);
    expect(claim).not.toBeNull();
    await pool.query(`update execution.execution_attempts
...
//# sourceMappingURL=lease-recovery.test.js.map