...
function imageStorageRequest() {
  return {
    ...
    ownerStorageAccess: {
      ...
      pendingResourceId: 'pending-image-output-reference',
      outputWriteGrantRef: 'write-grant-image-output-reference',
      artifactKind: 'image',
...
test('successful media execution persists provider output and authorization references', async () => {
  ...
  const stored = await pool.query<{
    ...
    `select safe_provider_output_ref, output_authorization_ref
